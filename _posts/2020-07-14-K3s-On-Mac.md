...
kubectl은 미리 Mac에 설치 되어 있었고, 없다면 설치 해야 Mac에서 kubectl을 바로 사용할 수 있다.
-->
...
<!-- 인증서 3개 모든 vm 노드 카피 후 인증서 적용 on k3s nodes 
...
```xml
<!--
k3s master는 /etc/rancher/k3s 폴더가 있지만, worker 노드는 k3s 폴더가 없다..없어서 만듬, 
worker에 없어도 될거 같은데 rancher 메뉴얼에서는 각 노드라고 표현되어 있어서 모든 노드에 생성 
아래는 k3s nodes 3개 모두 동일하게 만들어 준다.

k3s에서 프라이빗 레지스트리에서 이미지를 가져와 배포하게 하려면 이 mirrors 지시자가 필요한데
...
매핑된 Ingress로 배포된 어플리케이션을 확인해 볼 수 있다.
-->
...
#### 참고한 문서
`k3s` : [ks3 doc](https://rancher.com/docs/k3s/latest/en/)
...
`k3s github` : [k3s official github](https://github.com/rancher/k3s)
